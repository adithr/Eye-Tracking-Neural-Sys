...
def draw_display(dispsize, imagefile=None):
	...
	#--commented Adith 22 Apr ###
	# _, ext = os.path.splitext(imagefile)
	# ext = ext.lower()
	# data_type = 'float32' if ext == '.png' else 'uint8'
	screen = numpy.zeros((dispsize[1],dispsize[0],3), dtype='float32')#data_type)
	...
	if imagefile != None:
		...
		x = int((dispsize[0] - w)/2)
		y = int((dispsize[1] - h)/2)
		# draw the image on the screen
		screen[y:y+h,x:x+w,:] += img[:,:,:3]
	# dots per inch
	dpi = 100.0
	# determine the figure size in inches
	figsize = (int(dispsize[0]/dpi), int(dispsize[1]/dpi))
	# create a figure
